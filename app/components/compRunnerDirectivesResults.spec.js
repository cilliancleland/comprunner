...
describe('compRunnerDirectives: scResults', function() {
  let injector;
  let element;
  let scope;
  beforeEach(function() {
    injector = angular.injector(['ng','compRunner','app/views/results.html']);
  ...
  it('has one row for each player', function() {
    ...
    expect(trs[0].innerText.replace(/\s+/g, '') ).toEqual('NameArmyScore(Countback)');
